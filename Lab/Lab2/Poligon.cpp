...
ostream& operator<< (ostream& os, const Poligon& p) {
	os << "Temena poligona: ";
	for (int i = 0; i < p.brojTemena; ++i) {
		os << "(" << p.xKoordinate[i] << ", " << p.yKoordinate[i] << ") ";
	}
	return os;
}
...
istream& operator>> (istream& is, Poligon& p) {
	for (int i = 0; i < p.brojTemena; ++i) {
		...
		is >> p.xKoordinate[i] >> p.yKoordinate[i];
	}
	return is;
}
...
void Poligon::preslikajXKoordinate() {
	for (int i = 0; i < brojTemena; ++i) {
		yKoordinate[i] = -yKoordinate[i];
	}
}

void Poligon::preslikajYKoordinate() {
	for (int i = 0; i < brojTemena; ++i) {
		xKoordinate[i] = -xKoordinate[i];
	}
}